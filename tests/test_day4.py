...
class TestBuildSystemPrompt:
    ...
    def test_prefers_apply_patch_for_targeted_edits(self):
        tools = [
            make_tool_schema("apply_patch", "Prefer this tool for most code edits."),
            make_tool_schema("file_write", "Use this only for full-file rewrites."),
        ]
        prompt = build_system_prompt(".", tools)
        assert "Prefer structured edits with apply_patch" in prompt
        assert "apply_patch" in prompt

    def test_mentions_graph_and_revert_tools(self):
        tools = [
            make_tool_schema("graph_neighbors", "Inspect related files."),
            make_tool_schema("revert_patch", "Undo a prior patch."),
        ]
        prompt = build_system_prompt(".", tools)
        assert "graph_neighbors" in prompt
        assert "revert_patch" in prompt
        assert "imported/importing files" in prompt
...
class TestBuildTaskPrompt:
    ...
    def test_mentions_apply_patch_preference(self):
        prompt = build_task_prompt("Fix X", "/repo")
        assert "Prefer apply_patch for targeted edits" in prompt

    def test_includes_target_verification_when_present(self):
        prompt = build_task_prompt("Fix X", "/repo", test_cmd="python -m pytest test_demo.py -q")
        assert "Target Verification" in prompt
        assert "python -m pytest test_demo.py -q" in prompt

    def test_mentions_graph_neighbors_after_failure(self):
        prompt = build_task_prompt("Fix X", "/repo")
        assert "graph_neighbors" in prompt

    def test_includes_excluded_paths_when_present(self):
        prompt = build_task_prompt("Fix X", "/repo", exclude_paths=["logs", "README.md"])
        assert "Excluded Paths" in prompt
        assert "logs" in prompt
        assert "README.md" in prompt

    def test_includes_target_files_when_present(self):
        prompt = build_task_prompt("Fix X", "/repo", target_files=["report.py", "scores.py"])
...
class TestRouter:
    ...
    def test_openai_provider_accepts_siliconflow_env_key(self, monkeypatch):
        monkeypatch.delenv("OPENAI_API_KEY", raising=False)
        monkeypatch.setenv("SILICONFLOW_API_KEY", "sk-siliconflow")
        with patch("llm.openai_compat.OpenAICompatBackend.__init__", return_value=None):
            backend = create_backend("openai", "deepseek-ai/DeepSeek-V4-Flash")
        from llm.openai_compat import OpenAICompatBackend
        assert isinstance(backend, OpenAICompatBackend)
...
class TestTextFallback:
    ...
    def test_text_fallback_prefers_apply_patch(self):
        backend = self._make_backend()
        backend._client.chat.completions.create.return_value = self._make_response(
            "TASK_COMPLETE: done"
        )
        ...
        system_content = backend._client.chat.completions.create.call_args[1]["messages"][0]["content"]
        assert "prefer apply_patch" in system_content.lower()
        assert '"tool": "apply_patch"' in system_content
