...
class TestRepoMapCache:
    ...
    def test_cache_key_changes_with_exclude_paths(self, tmp_path):
        repo = tmp_path / "repo"
        repo.mkdir()
        ...
        task_a = Task(task_id="ca", description="a", repo_path=str(repo), exclude_paths=["README.md"], max_steps=5)
        task_b = Task(task_id="cb", description="b", repo_path=str(repo), exclude_paths=["logs"], max_steps=5)

        with EventLog.create(task_a, log_dir=str(tmp_path / "logs")) as log_a:
            agent.run(task_a, log_a)
        cache_key_a = agent._repo_map_cache_key

        with EventLog.create(task_b, log_dir=str(tmp_path / "logs")) as log_b:
            agent.run(task_b, log_b)
        cache_key_b = agent._repo_map_cache_key

        assert cache_key_a == (str(repo), ("README.md",))
        assert cache_key_b == (str(repo), ("logs",))
