...
class EventLog:
    ...
    def log_action(self, step: int, action: Action, raw_content: str = "") -> None:
        ...
        tool_call_id = None
        if action.tool_call:
            tool_call_id = f"{step_id}:{action.tool_call.name}"
        self._append(Event(
    ...
    def log_repo_map(self, summary: str, trace: dict, budget: int) -> None:
        """记录 repo-map 摘要和可追踪文件块。"""
        self._append(Event(
            event_type=EventType.REPO_MAP,
            task_id=self._current_task_id,
            payload={
                "budget": budget,
                "summary": summary,
    ...
    def log_reflection(self, step: int, reason: str, prompt: str) -> None:
        ...
        self._append(Event(
            ...
            payload={
                "step": step,
                "step_id": f"step-{step:03d}",
                "reflection_id": f"step-{step:03d}:reflection",
                "reason": reason,
                "prompt": prompt,
            },
        ))
    ...
    for event in events:
        if event.event_type == EventType.ACTION:
            ...
            if event.event_type == EventType.TASK_FAILED:
                stats["failure_type"] = event.payload.get("failure_type")
                stats["failure_stage"] = event.payload.get("failure_stage")
