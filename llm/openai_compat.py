...
def _build_tool_description_for_text(tools: list[LLMToolSchema]) -> str:
    ...
    lines = [
        "## Available tools",
        "For code edits, prefer apply_patch for targeted changes.",
        "Use file_write only when you intend to replace an entire file.",
        "To call a tool, output ONLY a JSON block in this exact format:",
        '```json\n{"tool": "<tool_name>", "params": {<params>}}\n```',
        "",
        "Preferred edit example:",
        '```json\n{"tool": "apply_patch", "params": {"patch_type": "search_replace", "path": "src/foo.py", "search": "old", "replace": "new"}}\n```',
        "",
