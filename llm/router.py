...
_ENV_KEY_MAP: dict[str, tuple[str, ...]] = {
    ...
    "openai":    ("OPENAI_API_KEY", "SILICONFLOW_API_KEY"),
...
def _resolve_api_key(provider: str, explicit_api_key: str | None) -> str:
    """Resolve api_key from an explicit value or provider-specific env fallbacks."""
    if explicit_api_key:
        return explicit_api_key

    for env_var in _ENV_KEY_MAP.get(provider, ()):
...
def create_backend(
    ...
    if not resolved_key and provider != "ollama":
        env_var = " or ".join(repr(name) for name in _ENV_KEY_MAP.get(provider, ()))
        raise ValueError(
